...
import { Event, EventVenue, UniversityEvent } from '../entities';
...
export class EventImporter {
  constructor(
    ...
    private readonly venueResolver: VenueResolver,
  ) {}
  ...
  async createMissingEvents(
    ...
    for (const parsedEvent of parsedEvents) {
      ...
      const venueIds = await this.venueResolver.resolveForUniversity(
        db,
        universityId,
        parsedEvent.venues,
      );
  ...
  private buildEventCriteria(input: EventCriteriaInput): EventCriteria {
    ...
    if (input.parsedEvent.isRecurring) {
      ...
      criteria.date = input.parsedEvent.date ?? undefined;
    }
