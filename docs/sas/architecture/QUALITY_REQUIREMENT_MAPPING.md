...
|---|---|---|---|
| **NFR-Corr-1 - Supported timetable PDF extraction correctness** | Isolate source-specific extraction behind an extension point and validate canonical output at every system boundary. | The parser registry, University of Pretoria adapter, canonical result schemas, worker validation, and authenticated callback path implement the required boundaries. Complete, manually reviewed ground-truth comparisons now demonstrate the requirement for all nine supported UP acceptance fixtures. | `test_up_ground_truth.py` compares every complete canonical result with the reviewed manifest, calculates field-level precision and recall, checks for omitted and invented records/fields, and requires exact equality. The 2026-09-02 run achieved 100% precision and recall with zero omissions and inventions; CI reruns the Python parser suite. |
